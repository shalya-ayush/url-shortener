...
def get_admin_info(db_url: models.URL) -> schemas.URLInfo:
    ...
    admin_end_point = app.url_path_for("administration_info", secret_key=db_url.secret_key)
    db_url.url = str(base_url.replace(path=db_url.key))
    db_url.admin_url = str(base_url.replace(path=admin_end_point))
    return db_url
...
@app.get("/admin/{secret_key}", name="administration_info", response_model=schemas.URLInfo)
